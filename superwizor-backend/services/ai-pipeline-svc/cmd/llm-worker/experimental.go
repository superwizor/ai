...
// kosztuje najwiecej.
func experimentalBanner(modalityCode, ontologyVersion, reportLanguage string) string {
	l := strings.ToLower(strings.TrimSpace(reportLanguage))
	...
	// Kazdy tag niepolski dostaje wariant angielski — ta sama zasada co
	// chrome renderera: ostrzezenie MUSI byc zrozumiale dla odbiorcy
	// raportu, a angielski jest mianownikiem dla jezykow bez tlumaczenia.
	return fmt.Sprintf(
		"> **⚠ EXPERIMENT — unauthorized ontology (draft %s, modality %s).**\n"+
			"> This report was generated from an ontology version not yet approved "+
			"by experts. **Not for clinical use.**\n"+
			"> Content generated by an artificial-intelligence system (art. 50 AI Act).\n\n",
		ontologyVersion, modalityCode)
}
