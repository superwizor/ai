...
| 1.9 | 2026-08-31 | **T42b — relacje ciągłości i rozliczenie pracy domowej** (docs/67 §4): nowy etap **S2k** (prompt s2k/1.0.0) — parowanie deterministyczne w kodzie (bieżące twierdzenie × przeszłe twierdzenia tego samego konstruktu z PastContext, maks. 3 na konstrukt), osąd relacji {wzmacnia, osłabia, bez_związku} jedną zbiorczą rundą LLM z enumem w schemacie; „bez_związku” nie tworzy linku. Rozliczenie: przeszłe ustalenia „praca domowa klienta” (konstrukt z fact_kind_map.agreement_client) dostają werdykt trójstanowy; werdykt „wrócono” bez istniejącego dowodu DEGRADUJE do nie_wrocono i jest zliczany (DroppedLinks). Zapis: report_claim_links (migracja 000104; CHECK per kind). Powierzchnia DETERMINISTYCZNA w rendererze (linie „Kontynuacja: potwierdza/osłabia ustalenie z DD.MM”, „bez nowych danych”, blok rozliczenia) — S4 nie konsumuje linków, więc data i kierunek nie mogą się „poprawić” w syntezie. Fail-open: awaria S2k = raport bez linków (ContinuityFailed w telemetrii). |
