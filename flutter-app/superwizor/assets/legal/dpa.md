...
**Data wejścia w życie:** 01.06.2026
...
*   **ZWAŻYWSZY, ŻE** w ramach świadczenia usług określonych w Umowie Głównej, Administrator powierza Podmiotowi Przetwarzającemu do przetwarzania dane osobowe swoich Pacjentów, w tym dane szczególnych kategorii (dane dotyczące zdrowia); 
...
> Akceptacja Regulaminu poprzez zaznaczenie odpowiedniego pola wyboru (checkbox) podczas rejestracji w Aplikacji jest równoznaczna z akceptacją i zawarciem niniejszej DPA. 
...
3.  **Dane Osobowe** – wszelkie dane osobowe Pacjentów Administratora, w tym dane osobowe szczególnych kategorii (dane dotyczące zdrowia fizycznego i psychicznego), przetwarzane przez Podmiot Przetwarzający w imieniu Administratora w związku z wykonywaniem Umowy Głównej. 
...
3.  **Charakter i cel przetwarzania:** Wykonywanie operacji na Danych Osobowych niezbędnych do świadczenia na rzecz Administratora usług Aplikacji, zgodnie z funkcjonalnościami opisanymi w § 4 Regulaminu, w szczególności: 
    *   Przechowywanie nagrań audio (tymczasowo — maksymalnie 48 godzin od przesłania, po czym są automatycznie i trwale usuwane).
    *   Automatyczna transkrypcja nagrań audio z wykorzystaniem technologii Speech-to-Text (Chirp 3).
    *   Diaryzacja (identyfikacja i rozróżnienie mówców z użyciem neutralnych etykiet — „Osoba 1", „Osoba 2").
    ...
    *   Tworzenie i przechowywanie zaszyfrowanej pamięci klinicznej (RAG) — anonimizowanych podsumowań sesji służących zapewnieniu ciągłości terapeutycznej.
...
5.  **Kategorie osób, których dane dotyczą:** Pacjenci Administratora, zgodnie z definicją w § 2 pkt 9 Regulaminu. 
6.  **Czas trwania przetwarzania:** Dane Osobowe będą przetwarzane przez czas obowiązywania Umowy Głównej, zgodnie z § 12 Regulaminu, z zastrzeżeniem że:
    *   Nagrania audio są usuwane automatycznie w ciągu 48 godzin od przesłania (niezależnie od statusu Umowy).
    *   Po rozwiązaniu Umowy Głównej, pozostałe Dane Osobowe (transkrypcje, raporty, pamięć kliniczna) są oznaczane jako usunięte (soft delete) i trwale usuwane w ciągu 30 dni.
...
1.  **Przetwarzania na polecenie:** Przetwarzać Dane Osobowe wyłącznie na udokumentowane polecenie Administratora. Za udokumentowane polecenie uznaje się postanowienia Umowy Głównej i niniejszej DPA, a także bieżące działania i konfiguracje dokonywane przez Administratora w Aplikacji (np. inicjowanie nagrania sesji, tworzenie kartoteki Pacjenta, ustawianie preferencji raportów). 
...
3.  **Bezpieczeństwa przetwarzania (art. 32 RODO):** Wdrożyć i stosować odpowiednie środki techniczne i organizacyjne zapewniające stopień bezpieczeństwa odpowiadający ryzyku, w szczególności opisane w Części I, pkt 5 Polityki Prywatności, obejmujące:
    *   Szyfrowanie kopertowe (Envelope Encryption) z kluczami CMEK zarządzanymi w Cloud KMS z automatyczną rotacją co 90 dni.
    *   Przechowywanie danych wyłącznie w regionie europe-central2 (Warszawa) z Vertex AI w europe-west4 (Holandia) — wyłącznie w EOG.
    *   Izolację sieciową bazy danych (brak publicznego IP, dostęp przez VPC Connector).
    *   Dedykowane konta usługowe z minimalnymi uprawnieniami dla każdego mikroserwisu.
    *   Automatyczne usuwanie nagrań audio w ciągu 48 godzin (OLM).
    *   Mechanizm soft delete z 30-dniowym okresem retencji.
...
7.  **Usuwania lub zwrotu danych:** Po zakończeniu świadczenia usług (rozwiązaniu Umowy Głównej), w zależności od decyzji Administratora, usunąć lub zwrócić mu wszelkie Dane Osobowe oraz usunąć ich kopie, chyba że prawo nakazuje ich przechowywanie. Procedura usuwania danych po zakończeniu umowy jest następująca:
    *   **Nagrania audio:** usunięte automatycznie w ciągu 48 godzin od przesłania (niezależnie od statusu Umowy) — brak działań wymaganych.
    *   **Transkrypcje, raporty, pomiary HiTOP, pamięć kliniczna RAG:** oznaczone jako usunięte (soft delete) i trwale usunięte z bazy danych w ciągu 30 dni.
    ...
    *   **Zaszyfrowane kopie zapasowe:** dane mogą być obecne w zaszyfrowanych kopiach zapasowych Cloud SQL przez okres ich retencji (do 30 dni), po czym są automatycznie nadpisywane. Bez dostępu do Cloud KMS dane w kopiach zapasowych pozostają nieczytelne.
...
2.  Posiada odpowiednią podstawę prawną do przetwarzania Danych Osobowych Pacjentów i powierzenia ich Podmiotowi Przetwarzającemu, zgodnie z obowiązkiem opisanym w § 5 ust. 2 Regulaminu. W szczególności w przypadku przetwarzania danych dotyczących zdrowia (art. 9 ust. 1 RODO), Administrator zapewnia spełnienie przesłanki z art. 9 ust. 2 RODO.
3.  Wypełnił obowiązek informacyjny wobec osób, których dane dotyczą (zgodnie z art. 13 lub 14 RODO), w tym poinformował Pacjentów o fakcie nagrywania sesji i korzystaniu z Aplikacji. Podmiot Przetwarzający udostępnia w Części II Polityki Prywatności wzorcowe informacje, które mogą być wykorzystane przez Administratora w celu wsparcia go w realizacji tego obowiązku.
...
2.  **Lista Sub-procesorów:** Podmiot Przetwarzający zobowiązuje się do utrzymywania aktualnej listy Sub-procesorów. Na dzień wejścia w życie niniejszej DPA, lista Sub-procesorów jest następująca:

| Sub-procesor | Usługa | Przetwarzane dane | Lokalizacja |
|---|---|---|---|
| **Google Cloud Platform** | Cloud Run, Cloud SQL PostgreSQL, Cloud Storage, Cloud KMS, Pub/Sub, Secret Manager | Przetwarzanie backendowe i przechowywanie danych | europe-central2 (Warszawa, Polska) |
...
| **Google Firebase** | Authentication, Cloud Firestore, Cloud Storage, FCM | Tokeny auth, lustrzane statusy, zdjęcia profilowe | europe-central2 |
| **Stripe, Inc.** | Przetwarzanie płatności, zarządzanie subskrypcjami | Dane płatnicze, informacje o firmie | Infrastruktura Stripe (PCI DSS Level 1) |
...
5.  **Brak transferu do Państw Trzecich:** Podmiot Przetwarzający gwarantuje, że **nie przekazuje Danych Osobowych do Państw Trzecich** (poza EOG). Cała infrastruktura Podmiotu Przetwarzającego jest zlokalizowana w Europejskim Obszarze Gospodarczym (region europe-central2 — Warszawa oraz region europe-west4 — Holandia). Ograniczenie regionów jest wymuszone na poziomie polityki organizacyjnej GCP (constraint `gcp.resourceLocations`).
...
3.  Zasady zmiany DPA są analogiczne do zasad zmiany Regulaminu, opisanych w § 13 Regulaminu. O każdej zmianie Administrator zostanie poinformowany z co najmniej 14-dniowym wyprzedzeniem. 
...
5.  Sądem właściwym do rozstrzygania sporów będzie sąd właściwy miejscowo dla siedziby Podmiotu Przetwarzającego w Krakowie, zgodnie z § 14 ust. 2 Regulaminu.
